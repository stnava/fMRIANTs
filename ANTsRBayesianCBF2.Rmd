...
library(ANTsR)
library(RKRNS)
myPriorStrength<-80.0 # controls prior influence
...
for ( i in c(1:max(seg)) )
  tissuelist[[i]]<-antsImageRead(paste('data3/seg2pcaslprob',i,'.nii.gz',sep=''),3)
...
mask[ mask > 0 ]<-1
mask2<-getMask(avg,mean(avg),Inf,1)
...
```{r perfpro, echo=TRUE,warning=F,message=F,cache=FALSE}
  perfpro <- aslPerfusion( pcasl, interpolation="linear",
        dorobust=0, useDenoiser=c(2), skip=11, 
        moreaccurate=motionAcc, verbose=1, mask=mask, ncompcor=0 ) 
...
Compute expected cbf $\Pr(\text{CBF}|\text{tissue})\Pr(\text{tissue})$.
...
#      , regweights=perfpro$regweights )
    locbeta<-blm$beta[1]
    bayesianperfusionloc[i]<-bayesianperfusionloc[i]+
      locbeta*tissueprior
    }
  }
close(pb)
...
plotANTsImage(bcbf$meancbf,slices=myslices,axis=3,outname=bcbfnm)
bcbf<-bcbf$meancbf
```
