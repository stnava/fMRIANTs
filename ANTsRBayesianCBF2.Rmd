...
## "Lesion" the asl if the lesion data exists

```{r lesion, echo=TRUE,cache=FALSE}
lesfn<-paste("data3/seg2pcasl",id,"lesion.nii.gz",sep='')
if ( file.exists(lesfn) )
  {
  lesionimg<-antsImageRead(lesfn,3)
  lesionimg[ seg > 0 ]<-lesionimg[ seg > 0 ]+1
  mask<-antsImageClone(seg)
  mask[ seg > 0]<-1
  mat<-timeseries2matrix( pcasl, mask )
  lesvec<-lesionimg[ mask >  0]
  lesmat<-mat
  inds<-(1:(nrow(mat)/2))*2
  lesmat[ inds, lesvec == 2 ]<-lesmat[ inds, lesvec == 2 ]*0.99
  pcasl<-matrix2timeseries(pcasl,mask,lesmat)
  }
```

# A non-expository wrapper function for this project

```{r bcbfun, echo=F,warning=F,message=F,cache=FALSE,eval=T}
...
antsImageWrite( bcbfl$bcbf , "/tmp/cbf.nii.gz")
```
...
```{r plotter2, echo=F,warning=F,message=F,cache=FALSE}
