...
```{r intvoxpriorvar, echo=FALSE, eval=TRUE}
for ( segval in 1:5 )
{
y=aslmat[,voxlist[segval]]
locdf<-data.frame(voxasl=y,
...
for ( rawcbf in c(F,T) ) {
  for ( usebayes in c(T,F) )
  {
    ...
    subcbf<-residuals( lm(
      ...
      preddf %>% ggvis(x = ~predAge, y = ~realAge, size := 100, opacity := 0.4,
        shape = ~factor(gen)) %>% layer_points()
...
preddf %>% ggvis(x = ~predFMem, y = ~realFMem, size := 100, opacity := 0.4,
  shape = ~factor(gen)) %>% layer_points()
print(paste("usebayes",usebayes,"useraw",rawcbf))
