...
ggvis(predListA[[3]], x = ~predAge, y = ~realAge,  opacity := 0.4) %>% layer_points(fill = ~factor(gen))  %>% group_by(gen) %>%  layer_model_predictions(model = "lm")  %>% knit_print()

print(predListAName[[4]])
ggvis(predListA[[4]], x = ~predAge, y = ~realAge,  opacity := 0.4) %>% layer_points(fill = ~factor(gen))  %>% group_by(gen) %>%  layer_model_predictions(model = "lm")  %>% knit_print()
```

## Population level results: Cognition

```{r pnccog, eval=TRUE, warning=F, message=F,cache=FALSE, echo=FALSE}
library(ANTsR)
library(e1071)
library(randomForest)
library(effects)
...
ggvis(predList[[3]], x = ~predFMem, y = ~realFMem,  opacity := 0.4) %>% layer_points(fill = ~factor(gen))  %>% group_by(gen) %>%  layer_model_predictions(model = "lm")  %>% knit_print()

print(predListName[[4]])
ggvis(predList[[4]], x = ~predFMem, y = ~realFMem,  opacity := 0.4) %>% layer_points(fill = ~factor(gen))  %>% group_by(gen) %>%  layer_model_predictions(model = "lm")  %>% knit_print()
```
