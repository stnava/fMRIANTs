...
predList[[1]] %>% ggvis(x = ~predAge, y = ~realAge, size := 100,  opacity := 0.4,  shape = ~factor(gen)) %>% group_by(gen) %>% layer_model_predictions(model = "lm")

##
print(predListName[[2]])
predList[[2]] %>% ggvis(x = ~predAge, y = ~realAge, size := 100,
  opacity := 0.4,
  shape = ~factor(gen)) %>% group_by(gen) %>% layer_model_predictions(model = "lm")

##
print(predListName[[3]])
predList[[3]] %>% ggvis(x = ~predAge, y = ~realAge, size := 100,
  opacity := 0.4,
  shape = ~factor(gen)) %>% group_by(gen) %>% layer_model_predictions(model = "lm")

##
print(predListName[[4]])
predList[[4]] %>% ggvis(x = ~predAge, y = ~realAge, size := 100,
  opacity := 0.4,
  shape = ~factor(gen)) %>% group_by(gen) %>% layer_model_predictions(model = "lm")
...
##
print(predListName[[1]])
predList[[1]] %>% ggvis(x = ~predFMem, y = ~realFMem, size := 100,
  opacity := 0.4,
  shape = ~factor(gen)) %>% group_by(gen) %>% layer_model_predictions(model = "lm")

##
print(predListName[[2]])
predList[[2]] %>% ggvis(x = ~predFMem, y = ~realFMem, size := 100,
  opacity := 0.4,
  shape = ~factor(gen)) %>% group_by(gen) %>% layer_model_predictions(model = "lm")

##
print(predListName[[3]])
predList[[3]] %>% ggvis(x = ~predFMem, y = ~realFMem, size := 100,
  opacity := 0.4,
  shape = ~factor(gen)) %>% group_by(gen) %>% layer_model_predictions(model = "lm")

##
print(predListName[[4]])
predList[[4]] %>% ggvis(x = ~predFMem, y = ~realFMem, size := 100,
  opacity := 0.4,
  shape = ~factor(gen)) %>% group_by(gen) %>% layer_model_predictions(model = "lm")
