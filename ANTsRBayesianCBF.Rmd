...
myPriorStrength<-150.0 # controls prior influence
...
```{r, echo=TRUE,warning=F,message=F,cache=FALSE}
getpriors<-function( img, seg, n )
  {
  p<-rep(0,n)
  for ( i in 1:n ) p[i]<-median( img[ seg == as.numeric(i) ] )
  return(p)
  }
intpriors<-getpriors( intimg, seg, max(seg) )
tissuepriors<-getpriors( perfimg, seg, max(seg) )
dvpriors<-getpriors( dvimg, seg, max(seg) )
```
