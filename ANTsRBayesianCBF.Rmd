...
myPriorStrength<-100.0 # controls prior influence
...
compcornuis<-compcor(raslmat,ncompcor)
...
```{r, echo=TRUE,warning=F,message=F,cache=FALSE}
 ...
 cbf <- quantifyCBF(  perfimg, mask,pcasl.parameters )$kmeancbf
 bcbf<- quantifyCBF( bperfimg, mask,pcasl.parameters )$kmeancbf
```

## The basic cbf map 
```{r, echo=F,warning=F,message=F,cache=FALSE}
cbfnm=paste(figpre,'cbf.png',sep='')
plotANTsImage(cbf,slices='3x6x1',axis=3,outname=cbfnm)
```
...
plotANTsImage(bcbf,slices='3x6x1',axis=3,outname=bcbfnm)
```
